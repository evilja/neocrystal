...
use crate::modules::{
    ...
    crystal_manager::crystal_manager,
};
fn main() {
    // establish communications and threads, then give the job to crystal_manager fn
    init_locale();
    ...
    thread::spawn(move || match play_audio(rx, tx_proc) {
        Ok(_) => (),
        Err(_) => (),
    });
